...
### 4.4 个别 PDF 的斜杠被抽成等号（字体编码问题，已知不处理）

**症状**

某篇论文的标题原文是 `Graphene/WSe2`，抽取出来却变成 `Graphene=WSe2`；译文里也照样是 `=`。

**根因**：这是 PDF 内嵌字体的编码映射问题——该字体没有提供正确的 `ToUnicode` 映射（或映射本身有误），pdf.js 抽文字时拿不到真正的字符码位，只好按字形索引回退，于是 `/` 变成了 `=`。**与分栏、段落重建无关，也与翻译无关**：原文侧就已经是错的，翻译只是忠实地保留了它。

**现状：已知且不处理。** 通用解法需要做字形反查（把嵌入字形的轮廓与已知字体比对），代价极高，而受影响的只是个别字符。

**怎么判断是这一类问题**：译文里出现"翻译没做错、但读起来不对"的符号时，先去看**原文那一段**。如果原文就已经错了，那就是抽取问题，不是翻译问题——不要去改提示词。

**可考虑的廉价缓解（未做）**：导入时统计异常字符比例，超过阈值就在论文详情里提示"这篇 PDF 的字体编码有问题，个别符号可能不准"。
...
### 7.6 设置里配好了、测试连接也通过，翻译却说"还没有选择配置"

**症状**

设置页里填好 Base URL、模型、密钥，点"测试连接"显示成功；回到论文页点"全文翻译"却报：

```
Error invoking remote method 'translate:start': Error: 还没有选择 AI 配置，请先到设置里添加
```

**根因：有两份状态，其中一份从来没人写。**

配置文件里存着两样东西：`ai-profiles.json` 是**配置列表**，`config.json` 里的 `activeProfileId` 是**当前使用哪一套**。设置页保存时只写了列表，**从来没有代码去写 `activeProfileId`**；而翻译入口读的正是后者，所以它永远是 `null`。

**为什么"测试连接"会掩盖它**：测试连接是直接拿配置 id 去调模型，压根不经过"当前配置"这一层。它能成功，反而让人以为配置已经就绪。

**这个 bug 单元测试为什么没抓到**：每个部件都有测试（配置读写有、翻译引擎有、设置页有），但**没有一条测试穿过"保存配置 → 开始翻译"这个集成点**。各自为政的绿灯掩盖了断掉的接线。

**修复**（三处一起改）

1. 设置页保存时同步写入 `activeProfileId`，并在页首显示"当前使用：xxx"，让状态可见。
2. 新增 `resolveActiveProfile()`：按 `activeProfileId` 找；找不到（没设置、或那套已被删除）就**退回第一套**，而不是直接报错。用户刚添加完配置就去点翻译是最常见的路径，这里必须给出可用结果。
3. 翻译入口与费用单价读取都改用它。

**预防**：凡是"列表 + 当前项"这种结构，都要问两句——**当前项是谁写的？写完有谁读？** 如果只有一个地方读、没有任何地方写，那就是这个 bug。

**更普遍的教训**：功能之间的**接线**必须有测试穿过。单元测试全绿不代表功能可用，只代表零件各自没问题。
